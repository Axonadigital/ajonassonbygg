...
class Media {
  ...
  createShader() {
    ...
    img.onload = () => {
      ...
      // Playfair Display laddas via next/font som sätter --font-playfair
      // till det interna familjenamnet – läs det för canvas-rendering
      const serif =
      ...
      if (this.subtext) {
        ctx.font = `600 ${Math.round(visH * 0.034)}px Inter, system-ui, sans-serif`;
        ctx.fillStyle = "rgb(198,176,142)";
        ctx.fillText(this.subtext.toUpperCase(), pad, visTop + visH - visH * 0.205);
      }

      ctx.font = `500 ${Math.round(visH * 0.07)}px ${serif}`;
      ctx.fillStyle = "rgb(245,240,232)";
      ctx.fillText(this.text, pad, visTop + visH - visH * 0.115);

      ctx.font = `400 ${Math.round(visH * 0.028)}px Inter, system-ui, sans-serif`;
      ctx.fillStyle = "rgba(245,240,232,0.6)";
      ctx.fillText("Klicka för att läsa mer", pad, visTop + visH - visH * 0.05);
